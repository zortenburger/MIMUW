...
struct Buffer {
    ...
    uint64_t size;    // Size of receiver_data in buffer.
    bool playing;   // True if the playing thread is running.
    bool *missing; // Array of missing packets.
    uint64_t last_missing; // Number of the last missing packet.
    uint64_t RTIME; // Time of the last packet received.
};

// Initializes a buffer.
struct Buffer *buffer_init(uint64_t BSIZE, uint64_t session_id, uint64_t byte0, uint64_t PSIZE, uint64_t RTIME) {
...
// Adds missing packets in the buffer that are before the last received packet.
void add_missing(struct Buffer *b, uint64_t last_received, struct Missing *missing) {
    uint64_t pop = 0;
    if (b->write_pos > b->size) {
        pop = b->write_pos - b->size;
    }
    uint64_t oldest_byte = MAX(b->last_missing, pop); // The oldest byte that have not been missing.


    uint64_t *missing_a = calloc(b->size / b->PSIZE, sizeof(uint64_t));
    ...
    for (; oldest_byte < last_received; oldest_byte += b->PSIZE) {
        ...
        if (b->missing[poz / b->PSIZE]) {
            missing_a[missing_size] = oldest_byte;
            missing_size++;
            b->last_missing = oldest_byte + b->PSIZE;
        }
    }
    if (missing_size > 0) {
        enMissing(missing, missing_a, missing_size, getCurrentTimeMillis() + b->RTIME, 0);
    }
}

// Puts receiver_data in the buffer.
void buffer_put(struct Buffer *b, uint8_t *data, uint64_t first_byte_num, struct Missing *missing) {

    CHECK(pthread_mutex_lock(&b->mutex));
    bool was_empty = false;

    if (first_byte_num < b->read_pos) {
    ...
    if (first_byte_num < b->write_pos && first_byte_num >= b->read_pos) { // It's the packet that was lost.
        ...
        while (b->write_pos <
               first_byte_num) { // Fill the buffer with zeros until the first byte of the packet and mark the packets as missing.
    ...
    add_missing(b, first_byte_num, missing);
...
// Prints PSIZE of receiver_data from the buffer.
bool buffer_get(struct Buffer *b) {
    ...
    if (b->missing[(b->read_pos % b->size) / b->PSIZE]) {
        pthread_mutex_unlock(&b->mutex);
        return true;
    }
